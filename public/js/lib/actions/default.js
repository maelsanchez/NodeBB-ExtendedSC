...
(function(Extendedsc) {
	var DefaultActions = {
		typing: function(sbInstance) {
			this.register = function() {
				sbInstance.dom.container.find('.extendedsc-message-input')
		...
		overlay: function(sbInstance) {
			this.register = function() {
				sbInstance.dom.overlay
					.off('click.overlay', '.extendedsc-content-overlay-close')
					.on('click.overlay', '.extendedsc-content-overlay-close', handle);
			};
		...
		scrolling: function(sbInstance) {
			this.register = function() {
				...
				sbInstance.dom.overlay
					.off('click.overlay', '#extendedsc-content-overlay-scrolldown')
					.on('click.overlay', '#extendedsc-content-overlay-scrolldown', function(e) {
			...
			function handle() {
				var shoutContent = sbInstance.dom.shoutsContainer,
					...
					scrollHeight = Extendedsc.utils.getScrollHeight(shoutContent),
		...
		delete: function(sbInstance) {
			this.register = function() {
				sbInstance.dom.container
					.off('click.delete', '.extendedsc-shout-option-close')
					.on('click.delete', '.extendedsc-shout-option-close', handle);
			};
			...
			this.register = function() {
				function eventsOff() {
					sbInstance.dom.shoutsContainer
						.off('click.edit', '.extendedsc-shout-option-edit')
				...
				function eventsOn() {
					sbInstance.dom.shoutsContainer
						.on('click.edit', '.extendedsc-shout-option-edit', function() {
							handle(
					...
					sbInstance.dom.textInput.on('keyup.edit', function(e) {
						if (e.which === 38 && !$(this).val()) {
							handle(
								sbInstance.dom.shoutsContainer
									.find('[data-uid="' + app.user.uid + '"].extendedsc-shout:last')
									.data('sid')
							);
						}
					});
	...
	for (var a in DefaultActions) {
		...
		Extendedsc.actions.register(a, DefaultActions[a]);
	}
