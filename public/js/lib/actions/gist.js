...
(function(Extendedsc) {
	var Gist = function(sbInstance) {
		this.register = function() {
			ajaxify.loadTemplate('extendedsc/features/gist', function(tpl){
				$(document.body).append(tpl);

				var gistModal = $('#extendedsc-modal-gist');

				sbInstance.dom.container.find('.extendedsc-button-gist').off('click').on('click', function(e) {
					gistModal.modal('show');
				});

				gistModal.find('#extendedsc-button-create-gist-submit').off('click').on('click', function(e) {
	...
	Extendedsc.actions.register('gist', Gist);
})(window.Extendedsc);