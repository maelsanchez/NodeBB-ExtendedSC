...
(function(Extendedsc) {
		...
		function parse(settings) {
			...
			for (var key in settings) {
				if (settings.hasOwnProperty(key)) {
					...
					var el = settingsMenu.find('[data-extendedsc-setting="' + key + '"] span');
	...
	function prettyString(key) {
		return key.replace('extendedsc:', '').replace(/:/g, '.');
	}

	function formalString(key) {
		return 'extendedsc:' + key.replace(/\./g, ':');
	}
