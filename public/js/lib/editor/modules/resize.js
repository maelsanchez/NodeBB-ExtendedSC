...
define('editor/resize', ['taskbar'], function(taskbar) {
	var resize = {};
	var oldRatio = 0;
	...
	function getSavedRatio() {
		return localStorage.getItem('editor:resizeRatio') || 0.5;
	}

	function saveRatio(ratio) {
		localStorage.setItem('editor:resizeRatio', Math.min(ratio, 1));
	}
	...
	function doResize(postContainer, ratio) {
		...
		if (bounds.width >= mediumMin) {
			...
			// the user can still scroll to the last post w/ editor open
	...
	if (raf) {
		resizeIt = function(postContainer, ratio) {
			raf(function() {
				...
				setTimeout(function () {
					$window.trigger('action:editor.resize');
					postContainer.trigger('action:editor.resize');
				}, 0);
			});
		};
	}
