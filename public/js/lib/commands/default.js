...
(function(Extendedsc) {
	...
	function soundCommand(sound, description) {
		return {
			...
			register: function(sbInstance) {
				sbInstance.sockets.registerMessage(sound, 'plugins.extendedsc.' + sound);
				sbInstance.sockets.registerEvent('event:extendedsc.' + sound, function() {
	...
	for (var c in DefaultCommands) {
		if (DefaultCommands.hasOwnProperty(c)) {
			Extendedsc.commands.register(c, DefaultCommands[c]);
		}
	}
